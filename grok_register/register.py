...
import cpa as _cpa
from config import config, load_config, save_config, PROVIDER_OPTIONS
import email_bridge
...
def get_email_and_token(api_key=None, log_callback=None):
    email_bridge.reset_provider()
    return email_bridge.get_email_and_token(config, log_callback=log_callback)
...
def get_oai_code(
    ...
    return email_bridge.get_oai_code(
        email,
        timeout=timeout,
...
class GrokRegisterGUI:
    ...
    def setup_ui(self):
        ...
        self.email_provider_combo = tk_option_menu(config_frame, self.email_provider_var, PROVIDER_OPTIONS, width=14)
        ...
        add_field(self.imap_user_entry, 2, 3)

        add_label(3, 0, "IMAP 授权码:")
        ...
        add_field(self.imap_password_entry, 3, 1)

        add_label(3, 2, "后缀文件:")
        self.suffixes_file_var = tk.StringVar(value=config.get("email_suffixes_file", "../email_suffixes.json"))
        self.suffixes_file_entry = tk_entry(config_frame, textvariable=self.suffixes_file_var, width=34)
        add_field(self.suffixes_file_entry, 3, 3)

        add_label(4, 0, "ChatTempMail API Key:")
        self.chat_tempmail_key_var = tk.StringVar(value=config.get("chat_tempmail_api_key", ""))
        self.chat_tempmail_key_entry = tk_entry(config_frame, textvariable=self.chat_tempmail_key_var, width=34)
        add_field(self.chat_tempmail_key_entry, 4, 1, columnspan=3)
    ...
    def start_registration(self):
        ...
        config["imap_password"] = self.imap_password_var.get().strip()
        config["email_suffixes_file"] = self.suffixes_file_var.get().strip()
        config["chat_tempmail_api_key"] = self.chat_tempmail_key_var.get().strip()
        ...
        save_config()
        if config["email_provider"] == "domain-imap":
            if not config.get("imap_user") or not config.get("imap_password"):
                self.log("[!] domain-imap 模式需要填写 IMAP 邮箱和授权码")
                return
        try:
