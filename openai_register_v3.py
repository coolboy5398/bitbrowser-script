...
def exchange_code_for_token(
    *,
    ...
    config = _backfill_token_fields(config)
...
def _build_compat_id_token(
    *,
    ...
    """构造一个最小兼容 JWT id_token，让老 CPA 能正常解析 chatgpt_account_id 等字段。
    结构: header.payload.signature（不签名，signature 为空）"""
    ...
    if chatgpt_account_id:
        auth_block["chatgpt_account_id"] = chatgpt_account_id
    if chatgpt_user_id:
        auth_block["chatgpt_user_id"] = chatgpt_user_id
    if plan_type:
        auth_block["plan_type"] = plan_type
    if organization_id:
        auth_block["organization_id"] = organization_id
    if auth_block:
        payload["https://api.openai.com/auth"] = auth_block
    h = _b64url_no_pad(json.dumps(header, separators=(",", ":")).encode("utf-8"))
    p = _b64url_no_pad(json.dumps(payload, separators=(",", ":")).encode("utf-8"))
    return f"{h}.{p}."


def _backfill_token_fields(config: Dict[str, Any]) -> Dict[str, Any]:
    """补齐 CPA 所需的全部字段，保证 id_token / credentials 层都有完整信息。

    处理逻辑：
    1. 从 access_token JWT 解析 chatgpt_account_id / chatgpt_user_id / exp
    2. 如果没有真实 id_token，生成兼容 id_token
    3. 补齐顶层 + credentials 子对象的所有字段
    """
    access_token = str(config.get("access_token") or "").strip()
    id_token = str(config.get("id_token") or "").strip()
    session_token = str(config.get("session_token") or "").strip()
    email = str(config.get("email") or "").strip()

    # 从 access_token JWT 解析关键字段
    at_claims = _jwt_claims_no_verify(access_token) if access_token else {}
    at_auth = at_claims.get("https://api.openai.com/auth") or {}
    at_exp = _to_int(at_claims.get("exp"))

    chatgpt_account_id = str(at_auth.get("chatgpt_account_id") or config.get("account_id") or "").strip()
    chatgpt_user_id = str(at_auth.get("chatgpt_user_id") or "").strip()
    plan_type = str(at_auth.get("plan_type") or "free").strip()
    organization_id = ""

    # 尝试从已有 id_token 解析 organization_id
    if id_token:
        ...
        if not organization_id:
            ...
            if orgs:
                organization_id = str((orgs[0] or {}).get("id", "")).strip()
        # 从 id_token 补充缺失字段
        if not chatgpt_account_id:
            chatgpt_account_id = str(it_auth.get("chatgpt_account_id") or "").strip()
        if not chatgpt_user_id:
            chatgpt_user_id = str(it_auth.get("chatgpt_user_id") or "").strip()
        if not email:
            email = str(it_claims.get("email") or "").strip()

    # 如果没有真实 id_token，生成兼容 id_token
    if not id_token and (chatgpt_account_id or email):
        exp_val = at_exp or (_to_int(time.time()) + 864000)
        ...
        print(f"[*] 已生成兼容 id_token (email={email}, account_id={chatgpt_account_id[:12]}...)")

    # 补齐顶层字段
    config["id_token"] = id_token
    config["account_id"] = chatgpt_account_id or config.get("account_id", "")
    config["chatgpt_account_id"] = chatgpt_account_id
    config["chatgpt_user_id"] = chatgpt_user_id
    if session_token:
...
class HybridOpenAIRegister:
    ...
    def extract_token_from_chatgpt_session(self) -> Optional[str]:
        """快速路径：利用 create_account 后已建立的 session，
        直接从 /api/auth/session 拿 accessToken JWT，跳过完整 OAuth 重登。"""
        callback_url = self._callback_url
        if not callback_url:
            print("[*] session fast path: 无 callback_url，跳过")
            return None

        # Step 1: GET callback URL 建立 ChatGPT session
        print(f"[*] session fast path: GET callback -> {_sanitize_url_for_log(callback_url)}")
        try:
            r = self.session.get(
                callback_url,
                headers={
            ...
            print(f"[*] session fast path: callback -> {r.status_code}")
        except Exception as e:
            print(f"[Warning] session fast path: callback 请求失败: {e}")
            return None

        _random_delay(0.3, 0.8)

        # Step 2: GET /api/auth/session 拿 accessToken
        print("[*] session fast path: GET /api/auth/session")
        try:
            r = self.session.get(
            ...
            if r.status_code != 200:
                print(f"[Warning] session fast path: /api/auth/session -> {r.status_code}")
                return None
        except Exception as e:
            print(f"[Warning] session fast path: /api/auth/session 请求失败: {e}")
            return None

        session_data = safe_json(r)
        access_token = str(session_data.get("accessToken") or "").strip()
        if not access_token:
            print("[Warning] session fast path: accessToken 为空")
            return None

        # Step 3: 解析 accessToken JWT
        claims = _jwt_claims_no_verify(access_token)
        email = str(claims.get("email") or session_data.get("user", {}).get("email") or "").strip()
        auth_claims = claims.get("https://api.openai.com/auth") or {}
        account_id = str(auth_claims.get("chatgpt_account_id") or "").strip()
        chatgpt_user_id = str(auth_claims.get("chatgpt_user_id") or "").strip()
        exp = _to_int(claims.get("exp"))

        if not email:
            print("[Warning] session fast path: 无法从 accessToken 解析 email")
            return None

        # Step 4: 提取 session_token（如有）
        session_token = ""
        jar = getattr(self.session.cookies, "jar", None)
        cookie_items = list(jar) if jar is not None else []
        for c in cookie_items:
            cname = getattr(c, "name", "") or ""
            if "session-token" in cname.lower() or "session_token" in cname.lower():
                session_token = str(getattr(c, "value", "") or "").strip()
                if session_token:
                    break

        now = int(time.time())
        expired_rfc3339 = format_beijing_from_epoch(exp if exp else now + 3600)
        now_rfc3339 = format_beijing_from_epoch(now)

        # Step 5: 构造 token config 并走 backfill
        config: Dict[str, Any] = {
            "id_token": "",
            "access_token": access_token,
        ...
        print(f"[*] session fast path: 成功! email={email}, account_id={account_id[:12]}...")
        return json.dumps(config, ensure_ascii=False, separators=(",", ":"))
...
def register_once(proxy: Optional[str], email_provider_name: Optional[str] = None) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        # 快速路径：直接从 ChatGPT session 获取 token（跳过完整 OAuth 重登）
        token_json = registrar.extract_token_from_chatgpt_session()
        if not token_json:
            print("[*] session 快速路径失败，降级为完整 OAuth 登录")
